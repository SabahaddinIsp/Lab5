...
void passGen(int a){
    if (a> 7 || a<3){
        while (a> 7 || a<3){
            cout<<"Number of words should be between 3-7(inclusive)";
            cin>>a;
            if(a <8 && a>2)
                break;
        }
...
bool search_letter(string a, char b){
    bool flag = false;
...
int main() {
    ...
    passGen(number);
    rep_with_upper(password);
    swap_letters(password);
